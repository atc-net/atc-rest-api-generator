...
public static class GenerateServerApiXunitTestEndpointTestHelper
{
        ...
        EndpointMethodMetadata endpointMethodMetadata)
    {
        ...
        if (!hostProjectOptions.ApiOptions.Generator.UseGlobalUsings)
        {
    ...
    private static void AppendTest400BadRequestInPath(
        StringBuilder sb,
        EndpointMethodMetadata endpointMethodMetadata,
        ResponseTypeNameAndItemSchema contractReturnTypeName)
    {
    ...
    private static void AppendActHttpClientOperation(
        int indentSpaces,
        StringBuilder sb,
    ...
    private static List<OpenApiParameter> FindBadRequestRouteParameters(
        List<OpenApiParameter> parameters)
        => parameters
            .Where(x => x.Schema.GetDataType() != OpenApiDataTypeConstants.String)
            .ToList();

    private static string RenderRelativeRefsForPathHelper(
        EndpointMethodMetadata endpointMethodMetadata,
        List<OpenApiParameter> allRouteParameters,
        List<OpenApiParameter> badRouteParameters,
        bool useForBadRequest)
    {
        ...
        var relativeRefPath = RenderRelativeRefPath(route, allRouteParameters, badRouteParameters, endpointMethodMetadata.ComponentsSchemas, useForBadRequest);
        ...
        var relativeRefQuery = RenderRelativeRefQuery(queryRequiredParameters, endpointMethodMetadata.ComponentsSchemas, useForBadRequest: false);
    ...
    private static string RenderRelativeRefPath(
        string route,
        List<OpenApiParameter> allRouteParameters,
        List<OpenApiParameter> badRouteParameters,
    ...
    private static string RenderRelativeRefQuery(
        List<OpenApiParameter> queryParameters,
        IDictionary<string, OpenApiSchema> componentsSchemas,
        bool useForBadRequest)
    {
    ...
    private static string PropertyValueGeneratorTypeResolver(
        OpenApiParameter parameter,
        IDictionary<string, OpenApiSchema> componentsSchemas,
        bool useForBadRequest)
    {
